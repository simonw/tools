...
def main():
    ...
    parser = argparse.ArgumentParser(description='Calculate sizes of Google Cloud Storage buckets')
    ...
    for bucket in buckets:
        ...
        print(f"Processed {lines_processed} lines, found {lines_with_size} files with size")
        print(f"{bucket}: {bucket_size_gb:.2f} GB")
        
        # Append summary to the listing file
        with open(listing_file, "a") as f:
            f.write(f"\n# SUMMARY\n")
            f.write(f"# Total size: {bucket_size_bytes} bytes ({bucket_size_gb:.2f} GB)\n")
